...
public class DynamoDbAccessService {
    ...
    public DeleteItemResponse deleteItem(String key) throws DynamoDBException {
        HashMap<String,AttributeValue> key_to_get = new HashMap<>();
        ...
        DeleteItemRequest test = DeleteItemRequest.builder().tableName(tableName).key(key_to_get).build();

        try {
            return dbClient.deleteItem(test);
        } catch (DynamoDBException e) {
            log.error("Error to get item from table {}: {}", tableName, e.getErrorMessage());
            throw e;
        }
